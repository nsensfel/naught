...
static inline void store_test (const uint8_t core_id, const int runs)
{
   register int i, j;
...
static inline void asm_evict_test (const uint8_t core_id, const int runs)
{
   register int i, j;
...
static inline void asm_load_evict_test (const uint8_t core_id, const int runs)
{
   register int i, j;
   register uint32_t buffer;
   ...
   for (i = 0; i < __NAUGHT_BENCH_RUNS; ++i)
   {
      ...
      switch (role)
      {
         ...
         case NAUGHT_INSTRUCTION_EVICT:
            asm_evict_test(core_id, ops_per_run);
            break;

         case 42:
            asm_load_evict_test(core_id, ops_per_run);
            break;
      }
