...
#define __NAUGHT_CORE_COUNT 3
#define __NAUGHT_BENCH_RUNS 1
#define __NAUGHT_BENCH_RUN_RESULTS (__NAUGHT_BENCH_RUNS * 4)

static naught_barrier_t operation_start =
   NAUGHT_BARRIER_INITIALIZED(__NAUGHT_CORE_COUNT);
...
(
   ...
   const int state_ix,
   const int target_state [const restrict static __NAUGHT_CORE_COUNT],
   const int operation,
   const int i,
...
{
   const int offset = ((core_id / 32) * 2);

   naught_barrier_join(&operation_start);

   if (core_id == 0)
   {
      naught_barrier_initialize(&operation_cleanup, __NAUGHT_CORE_COUNT);
      naught_barrier_initialize(&operation_end, __NAUGHT_CORE_COUNT);
   }
   ...
   switch (operation)
   {
      ...
      case NAUGHT_INSTRUCTION_LOAD_W_OFFSET:
         naught_benchmark_meta_read_elements_offset(elements_count, offset);
         break;
      ...
      case NAUGHT_INSTRUCTION_STORE_W_OFFSET:
         naught_benchmark_meta_write_elements_offset(elements_count, offset);
         break;
