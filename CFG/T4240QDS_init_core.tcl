...
# nsensfel: from _cacheon.tcl, required for L2 initialization.
variable CORES_PER_CLUSTER 	4
...
#	reg ${CAM_GROUP}L2MMU_CAM2  = 0x${DDR_TLB_SIZE}00000C1C08000000000000000000000000000000000001
    # nsensfel: Taking the init from _cacheon.tcl to make it cacheable.
	...
	##################################################################################
   #
  # nsensfel: Taken from _cacheon.tcl.
	# enable L2 cache per each cluster, only if not already enabled
	variable CLUSTER [expr [CORE $PIR] / $CORES_PER_CLUSTER]
	# L2 init: L2FI, L2LFC; L2CSR0
	variable L2_CACHE_ADDR [expr 0xC20000 + $CLUSTER * 0x40000]
	if {![expr [mem [CCSR_ADDR $L2_CACHE_ADDR] -np] & 0x80000000]} {
		# L2 init: L2FI, L2LFC; L2CSR0
		 mem [CCSR_ADDR $L2_CACHE_ADDR] = 0x00200400
		 while {[expr {[mem [CCSR_ADDR $L2_CACHE_ADDR] -np] != 0}]} {}
		 # L2 init: L2E; L2CSR0
		 mem [CCSR_ADDR $L2_CACHE_ADDR] = 0x80000000
